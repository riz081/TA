...
# Hand Detector
detector = HandDetector(detectionCon=0.8, maxHands=2)  # Allow detection of 2 hands
...
while True:
    ...
    if hands:
        for i, hand in enumerate(hands):
            if 'lmList' in hand and 'bbox' in hand:
                ...
                print(f'Primary Distance Hand {i+1}: {distanceCM} cm')
                ...
                # Debugging print statement
                print(f'Secondary Distance Hand {i+1}: {secondary_distanceCM} cm')

                # Display the distances on the camera feed
                cv2.putText(img, f'Hand {i+1} Primary: {int(distanceCM)} cm', (50, 50 + i*100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
                cv2.putText(img, f'Hand {i+1} Secondary: {int(secondary_distanceCM)} cm', (50, 100 + i*100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
    ...
    if key == ord("s"):
        ...
        for i in range(len(hands)):
            cv2.imwrite(f'{folder}/Image_{time.time()}_Hand_{i+1}.jpg', imgWhite)
        print(counter)
        print(lmList)
